...
import inc.uberpopug.accounting.repository.OutboxRepository
...
object OutboxRelay:
    ...
    ZLayer.scoped {
      for
        ...
        producer <- Producer.make(
          ProducerSettings(cfg.kafka.bootstrapServers).withClientId("ates-accounting-relay")
        )
      yield producer
    }
  ...
  val layer: ZLayer[OutboxRepository & Producer & AppConfig, Nothing, OutboxRelay] =
    ZLayer.fromFunction(OutboxRelayLive(_, _, _))
...
final case class OutboxRelayLive(
  ...
  private def topicFor(eventType: String): Option[String] =
    eventType match
      case PaymentProcessedBuilder.EventType => Some(cfg.kafka.topicPaymentProcessed)
      case _                                 => None
