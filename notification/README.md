...
- Telegram-канал: HTTP Bot API `sendMessage` (`java.net.http`, без доп.
  ...
  CircuitBreaker (5 неудачных вызовов send → open, 60s half-open) → TimeLimiter
  10s → Retry (exponential backoff, 5 попыток). Все параметры — в конфиге канала.
...
- Идемпотентность: `processed_events(event_id PK)` пишется ПОСЛЕ успешной доставки
  (read-check в начале) + `sent_notifications(event_id, channel, address)`
  вставляется ДО отправки (спека §7.4) — дубликаты не отправляются (дедуп по
  адресату, админская рассылка доходит каждому админу).
- Recovery: транзиентная ошибка не теряет событие — `processed_events` не записан,
  поток переподписывается и событие доставляется заново; при неудачной отправке
  маркер `sent_notifications` удаляется, поэтому DLQ-replay повторяет доставку.
  Исключение — краш между записью маркера и отправкой: маркер остаётся, событие
  не дублируется, но может быть потеряно.
